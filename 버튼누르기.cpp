  ...
  for(int i=0;i<n;i++)
  {
   ...
   dp[0][2]=color[0][2];
    ...
    for(int i=1;i<n;i++)
    {
      ...
      dp[i][0]=max(dp[i-1][1]+color[i][0],dp[i-1][2]+color[i][0]); 
      dp[i][1]=max(dp[i-1][0]+color[i][1],dp[i-1][2]+color[i][1]);
      dp[i][2]=max(dp[i-1][0]+color[i][2],dp[i-1][1]+color[i][2]);
    
      
    }
    // printf("%d %d %d\n",dp[n-1][0],dp[n-1][1],dp[n-1][2]);
  
    int result = max ( dp[n-1][2],max(dp[n-1][0],dp[n-1][1]));
    printf("%d",result);
